...
export default function V142() {
  ...
  // Detect touch devices (disable parallax & custom cursor)
  useEffect(() => {
    const touchDetected =
  ...
  // Thumbnail parallax hover (only on non-touch devices)
  ...
  // Custom glowing cursor follow (only non-touch)
  ...
  return (
    ...
    >
      {/* Custom glowing ring cursor (hidden on touch) */}
      <div
        ref={cursorRef}
        aria-hidden="true"
        style={{
        ...
        className={`cursor-glow ${isTouch ? "pointer-events-none" : ""}`}
      />

      {/* Banner Section (mobile-first: smaller height on small screens) */}
      <div
      ...
      >
        ...
        >
          {slides.map((s, i) => (
            ...
            >
              ...
              {/* Content container */}
              <div className="container mx-auto px-6 relative z-30 py-10">
                <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-4 items-center">
                  {/* Left Text (on mobile centered) */}
                  <div className="lg:col-span-7 flex flex-col justify-center h-full text-center lg:text-left">
                    <div className="mx-auto lg:mx-0 max-w-[720px]">
                      <h6 className="text-xs text-sm uppercase tracking-wider mb-3 font-medium text-amber-400">
                        {s.subtitle}
                      </h6>

                      {/* Responsive title sizes */}
                      <h1 className="text-[32px] sm:text-[40px] md:text-[56px] lg:text-[86px] xl:text-[96px] leading-[0.95] font-extrabold mb-4 sm:mb-6 text-white uppercase tracking-tight drop-shadow-[0_6px_0_rgba(0,0,0,0.35)]">
                        {s.title}
                      </h1>
                      ...
                      <div className="flex items-center justify-center lg:justify-start gap-4 mt-2">
                        <a
                          href={s.cta.href}
                          className="inline-flex items-center gap-3 bg-red-600 hover:bg-amber-400 hover:text-black text-white px-4 py-2 rounded-md font-semibold shadow-md transition-colors duration-200 text-sm"
                        >
                          <span>{s.cta.text}</span>
                          <span aria-hidden className="text-lg">→</span>
                        </a>
                        {/* On small devices optionally show a secondary action */}
                      </div>
                    </div>
                  </div>

                  {/* Right Thumbnail — desktop: large floating image; mobile: hidden here */}
                  ...
                  {/* Mobile thumbnail visible under text (centered) — border removed */}
                  <div className="lg:hidden col-span-1 mt-6">
                    <div className="flex justify-center">
                      <img
                        src={s.thumb}
                        alt="banner thumb mobile"
                        className="w-56 h-44 sm:w-64 sm:h-52 object-contain drop-shadow-xl"
                      />
                    </div>
                  </div>
                </div>
        ...
        <button
          ...
          className="absolute left-4 top-1/2 -translate-y-1/2 z-40 w-10 h-10 rounded-full bg-white/90 flex items-center justify-center shadow-md hover:scale-105 transition-transform"
        >
          ‹
        </button>
        <button
          ...
          className="absolute right-4 top-1/2 -translate-y-1/2 z-40 w-10 h-10 rounded-full bg-white/90 flex items-center justify-center shadow-md hover:scale-105 transition-transform"
        >
          ›
        </button>

        {/* Pagination (center bottom). On very small screens use smaller dots */}
        <div className="absolute bottom-2 left-1/2 -translate-x-1/2 z-40 flex items-center gap-3">
          {slides.map((_, i) => (
            <button
              key={i}
              onClick={() => goTo(i)}
              className={`w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center text-sm font-medium shadow-md transition-colors ${
      ...
      <style>{`
        ...
        /* small enhancement: smooth image rendering */
