...
/**
 * Simple marquee implemented with requestAnimationFrame so we avoid external CSS.
 * text: string to repeat
 * speed: pixels per second
 * className: tailwind classes for the container text size/color/etc
 */
function Marquee({ text, speed = 150, className = "text-white font-semibold" }) {
  ...
  useEffect(() => {
    ...
    // duplicate content so it appears continuous
...
export default function V231() {
  // Video & poster URLs (using https)
  const videoSrc =
  ...
  return (
    ...
    >
      ...
      {/* Dark overlay to mimic image-overlay opacity */}
      ...
      <div className="absolute top-125 left-0 right-0 z-20 pointer-events-none">
        <div className="max-w-full px-6">
          <Marquee
            ...
            speed={120} // px / sec
            className="text-[6rem] md:text-[8rem] lg:text-[6rem] leading-none text-white opacity-75"
          />
        </div>
      </div>

      {/* central overlay content (left-centered on large screens) */}
      <div className="absolute inset-0 z-30 flex items-center">
        <div className="container mx-auto px-6">
          <div className="max-w-2xl md:max-w-xl lg:max-w-2xl">
            <div className="text-left text-white">
              {/* heading */}
              <h2 className="font-serif text-3xl sm:text-4xl md:text-5xl lg:text-6xl leading-tight tracking-tight mb-4">
              ...
              <p className="text-sm sm:text-base md:text-lg text-white/90 mb-6">
                ...
                classic, elegant, <br className="hidden md:block" />
              ...
              <div className="flex items-center gap-4">
                <a
                  ...
                  className="inline-flex items-center gap-3 bg-white text-black font-medium py-2 px-4 rounded-md shadow-md hover:scale-[1.01] transition-transform"
                >
                  {/* svg icon */}
      ...
      {/* overlay content block in center-bottom (multi-line text + icons like the shopify markup) */}
      <div className="absolute mb-30 mt-150 left-0 right-0 z-30">
