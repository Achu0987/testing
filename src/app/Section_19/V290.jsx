    ...
    <div className="min-h-screen bg-[#f3efe6] bg-[radial-gradient(rgba(12,30,34,0.02)_1px,transparent_1px)] bg-[size:18px_18px] text-[#0b3742] font-[Montserrat]">
      <div className="max-w-[1200px] mx-auto px-5 py-9">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-7">
          <h1 className="font-[Bebas Neue] text-[44px] sm:text-[64px] tracking-[2px] text-[#214f57] leading-none text-left pl-[6px]">
            PRODUCTS
          </h1>
        ...
        {/* Product List */}
        <div
          className="flex sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-6 overflow-x-auto sm:overflow-visible pb-4 snap-x snap-mandatory"
          style={{ scrollSnapType: "x mandatory" }}
        >
          {products[activeTab].map((item, idx) => (
            <div
              key={idx}
              className="relative min-w-[85%] sm:min-w-0 rounded-xl p-7 min-h-[500px] flex flex-col items-center justify-start shadow-[0_8px_20px_rgba(10,20,30,0.06)] transition-all duration-200 hover:-translate-y-1 hover:shadow-[0_18px_36px_rgba(10,20,30,0.12)] snap-center"
              style={{
                ...
                {activeTab === "bottles" ? (
                  <>
                    ...
                    <img
                      ...
                      alt={item.name}
                      onClick={() => openModal(item)}
                      className="absolute w-[180px] cursor-pointer transform transition duration-300 opacity-0 group-hover:opacity-100"
                    />
                  </>
                ) : (
                  <img
      ...
      {modalData && (
        ...
        >
          <div className="relative bg-white rounded-2xl p-6 shadow-[0_24px_60px_rgba(10,20,30,0.3)] max-w-[90vw] sm:max-w-[600px] transition-all duration-500">
            <button
              onClick={closeModal}
