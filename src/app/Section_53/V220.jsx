  ...
  return (
    <section className="relative min-h-screen w-full bg-[#071a02] overflow-hidden flex flex-col items-center justify-center">
      ...
      <img
        ...
        alt="background"
        className="absolute inset-0 mt-40 w-full h-full "
      />
      ...
      {/* Heading */}
      <h2 className="text-white text-4xl md:text-6xl font-extrabold text-center mt-17 mb-2 tracking-wide z-30">
        OUR FLAVORS
      </h2>

      <div className="text-center mb-10 mt-10 z-30">
        <h3
      ...
      {/* Main Can + Objects */}
      <div className="relative flex items-center justify-center mb-10 z-30 px-4 w-full max-w-4xl">
        {/* LEFT DECORATION */}
        <img
          src={leftObjectSrc}
          alt={`${flavors[currentSlide].name} left decoration`}
          loading="lazy"
          className={`absolute left-1/2 -translate-x-[220%] bottom-15 ml-30 w-30 h-50 object-contain animate-float transition-transform duration-500 ${
        ...
        <img
          ...
          loading="lazy"
          className={`absolute ml-60 -translate-x-8 md:-translate-x-12 top-3 w-30 h-50 object-contain animate-float transition-transform duration-500 ${
            animate ? "scale-bounce" : ""
          }`}
          style={{ transformOrigin: "center" }}
        />
      ...
      <button
        ...
        className="absolute left-85 top-1/2 -translate-y-1/2 bg-white/10 hover:bg-white/25 text-white rounded-full p-3 md:p-4 z-40 transform transition-transform duration-200 shadow-md backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-white/40 hover:scale-105"
      >
        <svg
